...
def fetch_comments_with_stats(status_id):
    ...
    try:
        ...
        # 2. Get Context with headers (this removes the 60-post limit)
...
def sync_mastodon_comments():
    ...
    temp_db = "temp_publii_sync.sqlite"
    ...
    try:
        ...
        print("[*] Reading Publii database for recent posts...")
        # Smart Query: Posts from last 30 days OR the 5 most recently created posts
        query = """
        SELECT p.title, ad.value 
        FROM posts p
        JOIN posts_additional_data ad ON p.id = ad.post_id
        WHERE p.status = 'published' AND ad.key = 'postViewSettings'
        AND (
            p.created_at >= strftime('%s', 'now', '-30 days') * 1000
            OR 
            p.id IN (SELECT id FROM posts WHERE status = 'published' ORDER BY created_at DESC LIMIT 5)
        )
        """
        ...
        for title, settings_json in rows:
            try:
                ...
                m_id = m_id_obj.get('value') if isinstance(m_id_obj, dict) else None
        ...
        print(f"\n[✔] Success! {sync_count} JSON files updated.")
